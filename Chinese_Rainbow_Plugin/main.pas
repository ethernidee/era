...
begin
  ...
  if not fileexists(extractfilepath(strpas(path)) + 'hzk12') then begin
    ShowMessage('没有找到字体文件hzk12'); // Show error msg "Failed to find character lib hzk12"
    exit;
  end;
  
  if not fileexists(extractfilepath(strpas(path)) + 'HZK24') then begin
    ShowMessage('没有找到字体文件HZK24H'); // Show error msg "Failed to find character lib hzk24H"
    exit;
  end;
