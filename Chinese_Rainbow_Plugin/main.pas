...
(* Era *)
type
  TChineseGetCharColor = function (): integer; stdcall;
  TChineseGotoNextChar = procedure (); stdcall;
  
var
  ChineseGetCharColor: TChineseGetCharColor;
  ChineseGotoNextChar: TChineseGotoNextChar;
...
begin
  ...
  if not fileexists(extractfilepath(strpas(path))+'hzk12') then
  begin
    showmessage('没有找到字体文件hzk12');
    exit;
  end;
  if not fileexists(extractfilepath(strpas(path))+'HZK24') then
  begin
...
begin
  ...
  if (Buf[0]=$2d) and (Buf[1]=$49) and (Buf[2]=$9c) and
     (Buf[3]=$00) and (Buf[4]=$00) and (Buf[5]=$74) and
     (Buf[6]=$7c) and (Buf[7]=$83) then result:=32;
end;
...
procedure ConnectToEra;
var
  hEra: integer;

begin
  hEra := Windows.LoadLibrary('era.dll');
  Assert(hEra <> 0);
  ChineseGetCharColor := Windows.GetProcAddress(hEra, 'ChineseGetCharColor');
  Assert(@ChineseGetCharColor <> nil);
  ChineseGotoNextChar := Windows.GetProcAddress(hEra, 'ChineseGotoNextChar');
  Assert(@ChineseGotoNextChar <> nil);
end; // .procedure ConnectToEra
